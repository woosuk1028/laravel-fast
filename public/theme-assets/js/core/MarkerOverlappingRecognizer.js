...
var MarkerOverlappingRecognizer = function(opts) {
    ...
    this._options = $.extend({
        ...
        intersectListTemplate: '<div style="border:solid 1px #333;background-color:#fff;padding:5px;">'
            ...
            + '{{#repeat}}'
            + '<li style="list-style:none;margin:0;padding:0;"><a href="#" dat-lat="{{lat}}" dat-lng="{{lng}}">{{order}}. {{title}}</a></li>'
            + '{{/#repeat}}'
            + '</ul>'
            + '</div>'
    }, opts);
    ...
    _renderIntersectList: function(overlaped, offset) {
        ...
        for (var i=0, ii=overlaped.length; i<ii; i++) {
            ...
            item.on('click', function(e) {
                e.preventDefault();
                var lat = parseFloat($(this).find('a').attr('dat-lat'));
                var lng = parseFloat($(this).find('a').attr('dat-lng'));
                var position = new naver.maps.LatLng(lat, lng);

                self.getMap().panTo(position);
                setTimeout(function() {
                    self.getMap().setZoom(15, true);
                }, 1000);
            });
            // 추가한 코드 종료
